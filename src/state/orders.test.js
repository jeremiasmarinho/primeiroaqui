...
const cartState = {
  items: [
    { product: { id: 1, title: 'Produto A', price: 10 }, quantity: 2 },
    { product: { id: 2, title: 'Produto B', price: 5 }, quantity: 1 },
  ],
}
...
describe('orders state', () => {
  ...
  it('cria pedido com total baseado no carrinho', () => {
    ...
    const order = createOrder({
      cartState,
      delivery: { name: 'Ana', city: 'Centro', payment: 'Pix' },
      agentName: 'Joao',
      role: 'client',
  ...
  it('permite transicao valida Processando -> Em rota -> Entregue', () => {
    const base = { id: '1001', status: ORDER_STATUS.PROCESSING }
